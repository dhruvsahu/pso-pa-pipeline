...
#### Dashboard (batch visualization)

Navigate to **http://localhost:5000/dashboard** (or click the **Dashboard** tab in the top nav) to explore batch results visually. The dashboard reads `outputs/final_access_results.json` and displays:

- **KPI cards** — total policies, average/median score, score range, category counts
- **Score Distribution** — histogram colored by access category
- **Category Breakdown** — donut chart (Highly Restricted / Restricted / FDA Parity / Preferred)
- **Average Score by Brand** — horizontal bar chart ranking all 15 brands
- **Restriction Prevalence** — how many policies impose each restriction type
- **Step Therapy Counts** — grouped bar comparing brand vs generic step distributions
- **FDA Alignment** — donut chart of alignment categories
- **Top / Bottom Tables** — highest and lowest scoring policies at a glance

Run the batch pipeline first (`python run_full_pipeline.py`) to populate the data; the dashboard shows a helpful empty state otherwise.
...
│
├── templates/
│   ├── index.html                # Analyzer UI — upload PDF, stream results
│   └── dashboard.html            # Dashboard — batch results visualization (Chart.js)
│
├── docs/
│   ├── PIPELINE_FLOW.md          # Detailed pipeline walkthrough
│   └── ADR.md                    # Architecture Decision Records (16 decisions)
│
